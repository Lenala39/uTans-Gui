...
class Application:
    def __init__(self, master):
        ...
        def enable_Run():
            ...
            '''
            paths_set = (not (self.working_directory.get().strip())) and (
                not (self.volume_file.get().strip()))
            if (paths_set is True):
                self.button_run.config(state="disabled")
            else:
        ...
        def select_volume_file():
            ...
            if (not(self.volume_file.get().strip())):
                self.txt.insert(END, "Please select a Volume File \n")
        ...
        def select_working_dir():
            ...
            if (not(self.working_directory.get().strip())):
                self.txt.insert(END, "Please select a working directory \n")
        ...
        def check_config():
            '''
            if checkbox of config file is not set,
            button to select file is disabled
            '''
