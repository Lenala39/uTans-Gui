...
class Application:
    def __init__(self, master):
        ...
        self.txt.grid(row=11, column=0, sticky="nsew", padx=2, pady=2, columnspan=4)
        ...
        def select_volume_file():
            ...
            tmp = filedialog.askopenfilename(
                  initialdir="/home/lena/utans-filt/build", title="Volume Header File Select", filetypes=[("Volume Header", ".nhdr")])
        ...
        def select_config_file():
            ...
            '''
            tmp = filedialog.askopenfilename(initialdir="/home/lena/utans-filt/build/", title="Configuration File Select", filetypes=[("Configuration files", ".conf")])
        ...
        def run_application():
            ...
            if(self.up_thresh.get() <= self.low_thresh.get()):
                self.txt.insert(END, "ERROR: Lower threshold bigger than upper threshold")
            else:                
        ...
        def check_config():
            '''
            if checkbox of config file is not set, button to select file is disables
            ''' 

            if(self.use_config.get() == False):
